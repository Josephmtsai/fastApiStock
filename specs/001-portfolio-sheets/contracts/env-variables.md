...
| `GOOGLE_SHEETS_PORTFOLIO_GID` | 否* | `''` | `1004709448` | 持倉分頁 GID，取自 URL 中 `gid=` 後的數字 |
| `PORTFOLIO_CACHE_TTL` | 否 | `3600` | `7200` | Redis 快取 TTL（秒），建議 1800–7200 |

> \* 若兩者皆空，持倉功能靜默停用（推播正常，持倉區塊不顯示）
...
**GOOGLE_SHEETS_PORTFOLIO_GID**: 從 URL 的 `gid=` 參數取得，或點選分頁後看 URL 末尾
