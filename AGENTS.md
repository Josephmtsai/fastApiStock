...
## 7. Codex Skills Workflow & Handoff Protocol

### 流程

```
fastapistock-sa  ──handoff-sa.json──▶  fastapistock-developer  ──review──▶  fastapistock-reviewer  ──PASS──▶  fastapistock-qa
                                                                                                      ──FAIL──▶  fastapistock-developer (修正)
```

### 強制規則（Codex Orchestrator 必須遵守）

1. **fastapistock-developer 完成後，必須先使用 fastapistock-reviewer，等 review PASS 才能使用 fastapistock-qa。**
2. **fastapistock-reviewer 回報 FAIL 時，必須回到 fastapistock-developer 修正，禁止直接進 QA。**
3. fastapistock-sa 完成後才能啟動 fastapistock-developer（`handoff-sa.json` 的 `status` 必須為 `ready`）。
4. fastapistock-qa 完成後回報使用者，不自動 merge 或 deploy。
...
- **fastapistock-sa**：產出 `handoff-sa.json`，artifacts 必須列出所有 spec 文件路徑
- **fastapistock-developer**：產出 `handoff-dev.json`，changed_files 必須完整列出，告知 orchestrator 可以使用 **fastapistock-reviewer**（不是直接進 QA）
- **fastapistock-reviewer**：產出 review report，回報 PASS/FAIL verdict，PASS 才告知 orchestrator 可以使用 fastapistock-qa
- **fastapistock-qa**：不產出 handoff，直接以測試報告回報 orchestrator
